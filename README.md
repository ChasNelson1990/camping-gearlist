...
- `repair-kit.html` — its own checklist for the itemised gear repair kit contents, linked from the "Repair kit" entry on the main checklist.
...
`data/first-aid-kit.csv`, `data/repair-kit.csv`, or a sheet under
`data/research/`) in any spreadsheet app or text editor, then regenerate
the generated `.js` files:

    python3 scripts/extract_data.py

This overwrites `data.js`, `research-data.js`, `first-aid-data.js`, and
`repair-kit-data.js`. Commit both the CSV change and the regenerated `.js`
files together.
